 ...
 */
export function accrueOneEpochRewards(
  market: Market,
  ...
  marketEpoch.rewardsSupplyIndex = marketEpoch.rewardsSupplyIndex.plus(
    accrued.times(SHARES_OFFSET).div(market.supplyShares),
  );
 ...
 */
export function handleFullEpochs(
  market: Market,
  marketEpochFrom: MarketEpoch,
  marketEpochTo: MarketEpoch | null,
  situation: string,
): void {
  ...
  while (marketEpochToHandle !== marketEpochTo) {
    ...
    if (
      !marketEpochToHandle.totalRewardsAccrued.isZero() ||
      !marketEpochToHandle.rewardsSupplyIndex.equals(INITIAL_INDEX)
    ) {
...
export function accrueMarketRewards(marketId: Bytes, block: ethereum.Block): Market {
